...
> **BLOCKER and HIGH only.** MEDIUM / LOW / TECH_DEBT rules are in `.thesmos/RULES.md` —
> not embedded here to keep this file small. Run `thesmos explain <RULE_ID>` for full
> detail on any rule, including ones not listed below.
...
_Run `thesmos review` for findings. Run `thesmos validate` to gate CI._
...
Lean is not sloppy and god is not automatically a council — both tiers still route to the minimum specialist set that gets the task right; `god` only unlocks more ceremony and deeper escalation when the task actually warrants it.
---
