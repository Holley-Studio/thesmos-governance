...
| ~~`pantheon:export` generator is destructively out of sync with shipped exports~~ | **RESOLVED (Operation Sovereign Gate, 2026-07-06).** Verified by backed-up regeneration test: full `agents:export` run against the current catalog produced **68 agents/platform** (67 decreed + Asclepius, whose drop is incomplete and held back from bundles via `HOLDBACK_AGENT_IDS` in `package-agents.ts`) and round-tripped the enriched exports **byte-identical** — of 704 files, the only diff was the intended lean-ceremony template change in `buildCodexAgentsMd` propagating into `codex/AGENTS.md`. The fix arrived via PRs #70–#73 (dynamic agent discovery, path/target corrections, catalog-sourced models) which landed after this row was written. The `.claude/agents/`/`.cursor/rules/` side-effect install no longer exists in the code, and `git status` stays clean after a full regen. `pantheon:export` is safe to run again — god drops are unblocked. Pre-regen backup retained during verification. | Done | Claude, verified on `chore/roadmap-export-drift-resolved` |
