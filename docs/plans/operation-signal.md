...
| Committed machine-specific `/Users/...` paths in versioned config | **TO VERIFY** | `.thesmos/scope.json` (root, tracked) contains `"../../.claude/projects/-Users-MHolley-Desktop-thesmos-governance/memory/"` — this IS a machine/user-specific absolute-shaped path committed to a **tracked, versioned** file. Needs a decision: is this meant to be host-local (and thus should live in a gitignored override) or genuinely intended as a relative project path that happens to look absolute? |
| Health can report 100/A+ while `agents:doctor --strict` fails | **TO VERIFY** | Not yet checked against current `health.ts` / `agents:doctor` implementations. |
