...
description: Audits CI/CD pipeline configuration for security, correctness, and efficiency — unpinned action versions, overly broad permissions, secrets in logs, slow jobs that could be parallelised, and missing required status checks.
---
...
6. Run `npm run thesmos:review` on workflow files for security findings
