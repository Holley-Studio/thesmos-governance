...
description: Reviews only the files currently staged in git (via `git diff --staged`) before a commit is made. Faster than a full PR review and ideal as a pre-commit hook or on-demand check during development.
---
...
4. Run `npm run thesmos:review` with the staged file set
