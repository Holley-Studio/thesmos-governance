...
/**
 * Compact BLOCKER+HIGH-only table shared by every "thin" adapter target
 * (everything except the legacy full-catalog `agents` output, which is being
 * migrated separately). Embedding all ~1,137 rules with full descriptions and
 * code examples in every generated file was the #1 driver of oversized,
 * always-loaded adapters (measured: 130-160KB per file). MEDIUM/LOW/TECH_DEBT
 * rules and full rule text live in `.thesmos/RULES.md` — not duplicated here.
 */
function formatThinRulesTable(rules: Rule[]): string {
  const THIN_SEVERITIES: Severity[] = ['BLOCKER', 'HIGH'];
  const grouped = new Map<Severity, Rule[]>();
  for (const sev of THIN_SEVERITIES) grouped.set(sev, []);
  for (const rule of rules) {
    if (THIN_SEVERITIES.includes(rule.severity)) grouped.get(rule.severity)!.push(rule);
  }
  ...
  for (const sev of THIN_SEVERITIES) {
  ...
  parts.push('_Run `thesmos review` for findings. Run `thesmos validate` to gate CI._');
  return parts.join('\n').trimEnd();
}

export function generateGeminiRules(rules: Rule[], config: ThesmosConfig): string {
  return [
    `_Generated by Thesmos ${config.version} for **${config.project}**. Full rule catalog in \`.thesmos/RULES.md\`._`,
    '',
    'The following rules are enforced during code review. Violations marked **BLOCKER** must be fixed before merge.',
    '',
    formatThinRulesTable(rules),
  ].join('\n');
}
...
export function generateAgentsRules(rules: Rule[], config: ThesmosConfig): string {
  ...
  const restCount = rules.filter((r) => r.severity !== 'BLOCKER' && r.severity !== 'HIGH').length;
  ...
  if (blockers.length > 0) {
    ...
    for (const rule of blockers) {
      const summary = rule.description.length > 80 ? rule.description.slice(0, 77) + '…' : rule.description;
      parts.push(`| [${rule.id}] | \`${rule.category}\` | ${summary} |`);
    }
    parts.push('');
  }
  ...
  if (restCount > 0) {
    parts.push(
      `**GUIDELINES** — ${restCount} MEDIUM/LOW/TECH_DEBT rule(s) in \`.thesmos/RULES.md\`, not embedded here.`,
