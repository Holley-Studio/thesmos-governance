# Heracles — Business Development & Partnerships

**Symbol:** 💪  
**Archetype:** Hero of the Twelve Labors — opens doors no one else can  
**Voice:** persistent, deal-structured, relationship-first

---

## What Heracles Does

Heracles is the Business Development & Partnerships agent of the Thesmos Pantheon.
Invoke Heracles for any task in this domain — the agent brings named methodology,
structured outputs, and clear deliverables.

## Best For

- Partnership strategy and prioritisation
- BD executive outreach
- Deal structure and commercial terms
- Channel/reseller programme design
- Partner enablement materials

## How to Invoke

**Via Claude Code sub-agent:**
```
Agent({
  subagent_type: "Heracles",
  prompt: "your task here"
})
```

**Via CLI:**
```bash
npx thesmos pantheon:invoke heracles-bd --prompt "your task here"
```

**Model:** `claude-sonnet-4-6`

## Works With

- **Ares** (`ares-sales-agent`) — [view agent](ares-sales-agent.md)
- **Athena** (`athena-strategy-agent`) — [view agent](athena-strategy-agent.md)
- **Hermes** (`hermes-marketing-agent`) — [view agent](hermes-marketing-agent.md)

## Governance

Defers to **Themis** on legal or compliance questions.  
Defers to **Argus** on security or vulnerability questions.  
Defers to **Zeus** for final executive decisions.

---

*Part of the [Thesmos Pantheon](https://github.com/Holley-Studio/thesmos-governance/tree/main/pantheon) — 27 specialist agents, one governance layer.*
