...
const PASSING_CHECKS: DoctorCheck[] = [
  { name: 'config.json exists', pass: true, message: 'Found .thesmos/config.json' },
  { name: 'report.json exists', pass: true, message: 'Found .thesmos/report.json' },
];

const FAILING_CHECKS: DoctorCheck[] = [
  {
    name: 'thesmos:scan script',
    pass: false,
    message: 'Missing package.json script',
    fixHint: 'Add "thesmos:scan" to scripts in package.json',
  },
];
...
describe('formatAuditConsole', () => {
  ...
  it('shows fix hints for failed checks', () => {
    ...
    expect(out).toContain('thesmos:scan script');
    expect(out).toContain('Add "thesmos:scan"');
  });
