...
import { writeThesmosDir } from '../../init';
...
describe('runCiCheckForRoot — missing governance files', () => {
  ...
  it('fails when GitHub workflow is missing', () => {
    ...
    rmSync(join(root, '.github/workflows/thesmos-review.yml'), { force: true });

    const checks = runCiCheckForRoot(root, CONFIG_DEFAULTS);
    const wfCheck = checks.find((c) => c.name?.includes('thesmos-review.yml'));
