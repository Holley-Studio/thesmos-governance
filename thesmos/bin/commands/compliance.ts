...
/** Load scan evidence from `.thesmos/report.json` only — never invent an empty pass. */
function loadScanEvidence(root: string): ScanEvidence {
  ...
  return {
    ...
    evidenceSource: join(root, '.thesmos', 'report.json'),
  };
}
...
function toolVersionLabel(): string {
  return loadProductFacts().version;
}

function footerLine(): string {
  return `*Generated by [thesmos-governance](https://github.com/Holley-Studio/thesmos-governance) v${toolVersionLabel()}*`;
}

function scoreTableCell(assurance: AssuranceResult, passed: number, total: number): string {
  if (assurance.state === 'INCOMPLETE' || assurance.state === 'ERROR') {
    return `${assurance.state} (${formatAssuranceScore(assurance.score)}) — ${assurance.reason}`;
  }
  return `${formatAssuranceScore(assurance.score)} (${passed}/${total} rules passed) · ${assurance.state}`;
}
...
function generateGdprReport(root: string): ComplianceReportResult {
  ...
  const passed = evidence.evidenceMissing ? 0 : passedRules.length;
  const failed = evidence.evidenceMissing ? 0 : failedRules.length;
  const assurance = assuranceFromRuleCounts(passed, totalRules, {
    evidenceMissing: evidence.evidenceMissing,
    evidenceSource: evidence.evidenceSource,
    reason: evidence.evidenceMissing
  ...
  const lines: string[] = [
    ...
    `**Project:** ${projectName}`,
    `**Generated:** ${now}`,
    `**Tool:** thesmos-governance v${toolVersionLabel()}`,
    `**Assurance:** ${assurance.state}`,
    `**Rules Evaluated:** ${assurance.rulesEvaluated}`,
    `**Evidence:** ${evidence.evidenceSource ?? 'missing'}`,
    ``,
    `---`,
    ``,
    `## Executive Summary`,
    ...
    `|--------|-------|`,
    `| Assurance | ${assurance.state} |`,
    `| Compliance Score | ${scoreTableCell(assurance, passed, totalRules)} |`,
  ...
  if (assurance.state === 'INCOMPLETE' || assurance.state === 'ERROR') {
    lines.push(
      `> ○ **${assurance.state}.** ${assurance.reason}`,
      ``,
    );
  } else if (gdprFindings.length === 0) {
    lines.push(
    ...
    lines.push(
      `> ⚠️ **${failed} rule(s) failed** (${blockers.length} blockers, ${highs.length} highs).`,
      ``,
    );
  }
...
function generateFrameworkReport(
  ...
  for (const rule of rules) {
    ...
    const status = evidence.evidenceMissing
      ? '○ Incomplete'
      : hasFinding
        ? '❌ Failed'
        : '✅ Passed';
  ...
  lines.push(``, `---`, ``, `## Passed Rules (${assurance.rulesPassed})`, ``);
  if (evidence.evidenceMissing || passedRules.length === 0) {
    lines.push(evidence.evidenceMissing ? `*Not evaluated — scan evidence missing.*` : `*No rules passed.*`, ``);
  } else {
    for (const rule of passedRules) {
      lines.push(`- ✅ **${rule.id}** — ${rule.description}`);
    }
  ...
  if (ordered.length === 0) {
    lines.push(evidence.evidenceMissing ? `*Incomplete — no scan evidence.*` : `*No remediation required.*`, ``);
  } else {
    lines.push(`| Priority | Rule | File | Action |`, `|----------|------|------|--------|`);
    for (let i = 0; i < ordered.length; i++) {
      const f = ordered[i]!;
...
function generateEuAiActReport(root: string): ComplianceReportResult {
  return generateFrameworkReport(root, 'eu-ai-act', 'EU_AI_', 'eu_ai_', EU_AI_ARTICLE_MAP);
}

function generateHipaaReport(root: string): ComplianceReportResult {
  return generateFrameworkReport(root, 'hipaa', 'HIPAA_', 'hipaa_', HIPAA_ARTICLE_MAP);
}

function generateDoraReport(root: string): ComplianceReportResult {
  return generateFrameworkReport(root, 'dora', 'DORA_', 'dora_', DORA_ARTICLE_MAP);
}

function generateSoc2Report(root: string): ComplianceReportResult {
  const config = loadConfig(root);
  const evidence = loadScanEvidence(root);
...
function generateNistAiRmfReport(root: string): ComplianceReportResult {
  const config = loadConfig(root);
  const evidence = loadScanEvidence(root);
...
function buildSoc2NistReport(
  ...
  const passed = evidence.evidenceMissing
    ? 0
    : rules.filter((r) => !findings.some((f) => f.category === r.category)).length;
  ...
  const lines: string[] = [
    ...
    `|--------|-------|`,
    `| Assurance | ${assurance.state} |`,
    `| Coverage Score | ${scoreTableCell(assurance, passed, total)} |`,
    `| Total Findings | ${findings.length} |`,
    ``,
  ];

  if (assurance.state === 'INCOMPLETE' || assurance.state === 'ERROR') {
    lines.push(`> ○ **${assurance.state}.** ${assurance.reason}`, ``);
  } else if (findings.length === 0) {
    lines.push(`> ✅ **No violations detected in mapped rules.**`, ``);
  } else {
    lines.push(`> ⚠️ **${findings.length} violation(s) detected.**`, ``);
  }

  lines.push(`---`, ``, `## Findings by Control Area`, ``);
  ...
  for (const rule of rules) {
    ...
    const status = evidence.evidenceMissing
      ...
      : hasFinding
        ? '❌ Failed'
        : '✅ Passed';
    lines.push(`| ${rule.id} | ${area} | ${status} |`);
  }
...
export async function cmdCompliance(argv: string[]): Promise<void> {
  ...
  switch (standard as Standard) {
    case 'gdpr':
      result = generateGdprReport(root);
      break;
    case 'eu-ai-act':
      result = generateEuAiActReport(root);
      break;
    case 'hipaa':
      result = generateHipaaReport(root);
      break;
    case 'dora':
      result = generateDoraReport(root);
      break;
    case 'soc2':
      result = generateSoc2Report(root);
      break;
    case 'nist-ai-rmf':
      result = generateNistAiRmfReport(root);
      break;
  }
  ...
  if (write || output) {
    ...
    process.stdout.write(`Assurance: ${result.assurance.state} (${formatAssuranceScore(result.assurance.score)})\n`);
  } else {
    process.stdout.write(report);
  }

  process.exitCode = exitCodeForAssurance(result.assurance.state);
}
