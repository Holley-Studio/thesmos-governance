...
function workflowYaml(config: ThesmosConfig): string {
  ...
  return [
    ...
    '        run: npm ci',
    '',
    '      # First run after init: this scan populates the stub .thesmos/report.json.',
    '      # Subsequent runs refresh it — no manual step needed before the first PR.',
    '      - name: Thesmos — scan',
    ...
    '',
    '      # continue-on-error: the review output is informational context for the PR;',
    '      # the validate step below is the actual merge gate.',
    '      - name: Thesmos — review',
