...
## For each finding:
1. Understand the attack vector (run `thesmos explain <rule-id>`)
2. Apply the minimum fix — don't refactor surrounding code
3. Re-scan the file: `thesmos review <file>`
...
```
thesmos suppress --rule=<rule-id> --file=<file> --reason="<explanation>"
```
