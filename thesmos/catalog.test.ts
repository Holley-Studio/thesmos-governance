...
`;
    ...
    expect(frontmatter.owner).toBe('thesmos');
...
describe('loadBuiltInProfiles', () => {
  ...
  it('every profile includes @thesmos/core in rulePacks', () => {
    const profiles = loadBuiltInProfiles();
    for (const profile of profiles) {
      expect(profile.rulePacks, `${profile.id} must include @thesmos/core`).toContain(
        '@thesmos/core'
      );
    }
  });
...
`;
  ...
  function withTempDir(fn: (root: string) => void): void {
    const root = join(tmpdir(), `thesmos-catalog-test-${Date.now()}`);
    mkdirSync(root, { recursive: true });
    try {
      fn(root);
    } finally {
  ...
  it('returns empty catalog when .thesmos/agents and .thesmos/skills do not exist', () => {
    withTempDir((root) => {
  ...
  it('loads a custom agent file from .thesmos/agents/', () => {
    withTempDir((root) => {
  ...
  it('loads a custom skill file from .thesmos/skills/', () => {
    withTempDir((root) => {
...
describe('getActiveCatalog (integration)', () => {
  function withTempDir(fn: (root: string) => void): void {
    const root = join(tmpdir(), `thesmos-active-catalog-test-${Date.now()}`);
    mkdirSync(root, { recursive: true });
    try {
      fn(root);
    } finally {
