...
function makeTmpDir(): string {
  const dir = join(tmpdir(), `thesmos-ctx-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('generateContextCapsule', () => {
  ...
  it('reads project name from package.json', () => {
    writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'thesmos-helper' }));
    const capsule = generateContextCapsule(root);
    expect(capsule.project).toBe('thesmos-helper');
  });

  it('health score starts below 100 when no governance files exist', () => {
    const capsule = generateContextCapsule(root);
    // Missing .thesmos/config.json, CLAUDE.md, scope.json → deductions expected
...
describe('saveContextCapsule / loadContextCapsule', () => {
  ...
  it('saves context.md to .thesmos/', () => {
    const capsule = generateContextCapsule(root);
    saveContextCapsule(root, capsule);
    expect(existsSync(join(root, '.thesmos', 'context.md'))).toBe(true);
  });

  it('saves context-meta.json alongside context.md', () => {
    const capsule = generateContextCapsule(root);
    saveContextCapsule(root, capsule);
    expect(existsSync(join(root, '.thesmos', 'context-meta.json'))).toBe(true);
  });
  ...
  it('saved context.md contains the project name', () => {
    ...
    const capsule = generateContextCapsule(root);
    saveContextCapsule(root, capsule);
    const md = require('node:fs').readFileSync(join(root, '.thesmos', 'context.md'), 'utf8');
