...
describe('SELF_001 — version behind', () => {
  it('fires on thesmos-governance pinned to old minor without caret', () => {
    const findings = detect('SELF_001', [{
      path: 'package.json',
      content: JSON.stringify({
        name: 'my-app',
        devDependencies: { 'thesmos-governance': '2.1.0' },
      }, null, 2),
    }]);
  ...
  it('does NOT fire when using caret range', () => {
    const findings = detect('SELF_001', [{
      ...
      content: JSON.stringify({
        name: 'my-app',
        devDependencies: { 'thesmos-governance': '^2.3.1' },
      }, null, 2),
    }]);
    expect(findings).toHaveLength(0);
  });

  it('does NOT fire when thesmos-governance is not present', () => {
    const findings = detect('SELF_001', [{
      path: 'package.json',
...
describe('SELF_002 — exact version pin', () => {
  it('fires on exact semver without caret', () => {
    const findings = detect('SELF_002', [{
      ...
      content: JSON.stringify({
        name: 'my-app',
        devDependencies: { 'thesmos-governance': '2.3.0' },
      }, null, 2),
    }]);
    expect(findings.length).toBeGreaterThan(0);
...
describe('SELF_003 — broken hook', () => {
  it('fires on git hook with absolute thesmos binary path', () => {
    const findings = detect('SELF_003', [{
      path: '.git/hooks/pre-commit',
      content: '#!/bin/sh\n/home/user/.npm-global/bin/thesmos ci --hook=pre-commit\n',
    }]);
    expect(findings.length).toBeGreaterThan(0);
    expect(findings[0]?.severity).toBe('HIGH');
  });
  ...
  it('fires on macOS absolute path in husky hook', () => {
    const findings = detect('SELF_003', [{
      ...
      content: '#!/bin/sh\n/Users/matt/.nvm/versions/node/v20.0.0/bin/thesmos ci\n',
    }]);
    expect(findings.length).toBeGreaterThan(0);
  });

  it('does NOT fire on npx thesmos call', () => {
    const findings = detect('SELF_003', [{
      path: '.git/hooks/pre-commit',
  ...
  it('does NOT fire on non-hook files', () => {
    const findings = detect('SELF_003', [{
      ...
      content: '/usr/local/bin/thesmos ci\n',
    }]);
    expect(findings).toHaveLength(0);
  });
});
...
describe('SELF_005 — stale adapter', () => {
  it('fires on CLAUDE.md referencing old minor version', () => {
    const findings = detect('SELF_005', [{
      path: 'CLAUDE.md',
      content: '# Thesmos Governance\n\nthesmos-governance v2.1.0 — 911 rules active\n',
    }]);
    expect(findings.length).toBeGreaterThan(0);
    expect(findings[0]?.severity).toBe('MEDIUM');
  });
  ...
  it('does NOT fire when version is current (2.3.x)', () => {
    const findings = detect('SELF_005', [{
      path: 'CLAUDE.md',
      content: '# Thesmos Governance\n\nthesmos-governance v2.3.1 — 1075 rules active\n',
    }]);
    expect(findings).toHaveLength(0);
  });
});
...
describe('SELF_008 — CI pinned old version', () => {
  it('fires on npx thesmos-governance@old-version in CI workflow', () => {
    const findings = detect('SELF_008', [{
      path: '.github/workflows/ci.yml',
      content: 'steps:\n  - run: npx thesmos-governance@2.1.0 ci\n',
    }]);
    expect(findings.length).toBeGreaterThan(0);
  ...
  it('does NOT fire on current version', () => {
    const findings = detect('SELF_008', [{
      path: '.github/workflows/ci.yml',
      content: 'steps:\n  - run: npx thesmos-governance@2.3.1 ci\n',
    }]);
    expect(findings).toHaveLength(0);
  });
});
...
describe('SELF_009 — orphaned suppression', () => {
  it('fires on suppression with unknown rule prefix', () => {
    const findings = detect('SELF_009', [{
      ...
      content: '// thesmos-ignore: LEGACY_001\nconst x = 1;\n',
    }]);
    expect(findings.length).toBeGreaterThan(0);
    expect(findings[0]?.severity).toBe('MEDIUM');
  });
  ...
  it('does NOT fire on valid rule prefixes', () => {
    const findings = detect('SELF_009', [{
      path: 'src/api/route.ts',
      content: '// thesmos-ignore: JWT_001\nconst secret = process.env.JWT_SECRET || "fallback";\n',
    }]);
    expect(findings).toHaveLength(0);
  });
...
describe('SELF_010 — not in devDependencies', () => {
  it('fires when thesmos-governance is absent from package.json', () => {
  ...
  it('does NOT fire when thesmos-governance is in devDependencies', () => {
    const findings = detect('SELF_010', [{
      path: 'package.json',
      content: JSON.stringify({
        name: 'my-app',
