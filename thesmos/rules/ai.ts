...
export const AI_RULES: ThesmosRule[] = [
  ...
  },
  {
    ...
    explain: {
      why: 'EU AI Act Annex III classifies credit, hiring, and health decisions as high-risk AI. Without a mandatory human review gate, fully automated adverse decisions violate Art. 14 human oversight requirements and expose the organisation to regulatory action.',
      commonViolations: [
        'LLM returns a credit score or loan approval decision that is applied directly without human sign-off',
        'Hiring pipeline uses LLM scoring to auto-reject candidates with no human review step',
        'Health-risk assessment routes patients based on LLM output alone',
      ],
      goodExample: `// Flag for human review before acting
const risk = await openai.chat.completions.create({ model: 'gpt-4o', messages });
if (HIGH_RISK_DOMAINS.includes(domain)) {
  await queueForHumanReview({ riskScore: risk, requiresApproval: true });
} else {
  applyDecision(risk);
}`,
      badExample: `// Auto-apply LLM decision in high-risk domain
const hiringDecision = await openai.chat.completions.create({ messages: hiringMessages });
await updateCandidateStatus(candidateId, hiringDecision.choices[0]!.message.content);`,
    },
  },
  {
    id: 'AI_039',
    ...
    explain: {
      why: 'EU AI Act Art. 13 requires that AI systems deployed to end users include clear disclosure that output is AI-generated, preventing deceptive manipulation.',
      commonViolations: [
        'LLM-generated product descriptions displayed without "AI-generated" label',
        'Chatbot responses presented as human-authored support replies',
        'AI-written emails sent without disclosure in professional contexts',
      ],
      goodExample: `// Always disclose AI origin
res.json({ content: llmOutput, generatedBy: "AI", disclaimer: "This content was generated by AI and may require review." });`,
      badExample: `// No disclosure — user thinks it's human-written
res.json({ content: llmOutput });`,
    },
  },
  {
    id: 'AI_040',
    ...
    explain: {
      why: 'EU AI Act Art. 12 and HIPAA §164.312(b) require immutable audit logs of AI decisions for traceability, accountability, and post-incident investigation.',
      commonViolations: [
        'AI credit decisions stored in a mutable SQL table',
        'AI hiring scores updated in place without versioning',
        'Audit logs written to the same DB the AI model can write to',
      ],
      goodExample: `await appendAuditLog({ decision: aiOutput, model, timestamp: Date.now(), userId }); // append-only`,
      badExample: `await db.decision.update({ where: { id }, data: { result: aiOutput } }); // mutable — no trail`,
    },
  },
  {
    id: 'AI_041',
    ...
    explain: {
      why: 'Models used for classification or scoring can encode historical biases that systematically disadvantage protected groups. Without evaluation, bias goes undetected until legal action.',
      commonViolations: [
        'Hiring model trained on historical data without demographic parity check',
        'Credit scoring model deployed without disparate impact analysis',
        'Content moderation model applied without false-positive rate by group',
      ],
      goodExample: `// Run fairness evaluation before deployment
await evaluateFairness(model, { demographicParity: true, equalOpportunity: true });`,
      badExample: `// No bias check — deployed directly to production
const result = await model.predict(input);`,
  ...
  },
  {
    ...
    explain: {
      why: 'GDPR Art. 28 and HIPAA require a signed Data Processing Agreement (DPA) before PII can be sent to any third-party processor including external LLM APIs.',
      commonViolations: [
        'User emails sent to OpenAI without a signed DPA',
        'Patient notes submitted to Anthropic API without a Business Associate Agreement',
        'Employee reviews passed to Gemini without legal review',
      ],
      goodExample: `// Sanitize PII before sending to LLM
const sanitized = removePII(userContent);
const response = await openai.chat.completions.create({ messages: [{ role: "user", content: sanitized }] });`,
      badExample: `// Sending raw PII to external LLM
const response = await openai.chat.completions.create({ messages: [{ role: "user", content: userProfile.email + " " + userProfile.notes }] });`,
    },
  },
  {
    id: 'AI_043',
    ...
    explain: {
      why: 'Unexplainable AI outputs create legal liability under GDPR Art. 22 (right to explanation) and erode user trust in high-stakes domains.',
      commonViolations: [
        'Loan denial message "Application declined" with no reason given',
        'Medical risk score returned without feature attribution',
        'HR performance rating with no explanatory factors',
      ],
      goodExample: `res.json({ decision: "declined", explanation: await explainDecision(features), appealLink: "/appeal" });`,
      badExample: `res.json({ decision: "declined" }); // no explanation`,
    },
  },
  {
    id: 'AI_044',
    ...
    explain: {
      why: 'Without provenance tracking on training data, it is impossible to detect contamination, bias sources, or licensing violations after deployment.',
      commonViolations: [
        'Training pipeline ingests web scrapes without source URLs',
        'Fine-tuning data mixed from multiple origins without labelling',
        'Model trained on user-submitted content without consent records',
      ],
      goodExample: `await ingestTrainingDoc({ content, source: url, license, ingestedAt: Date.now(), consent: true });`,
      badExample: `await ingestTrainingDoc({ content }); // no provenance`,
    },
  },
  {
    id: 'AI_045',
    ...
    explain: {
      why: 'LLM output schema can drift silently between model versions causing runtime parsing failures in consuming services.',
      commonViolations: [
        'Response shape changes from {answer: string} to {response: string} after model update',
        'Structured output drops a required field on a model version bump',
        'JSON schema not re-validated after model upgrade',
      ],
      goodExample: `const parsed = OutputSchema.parse(await llm.generate(prompt)); // schema enforced every call`,
      badExample: `const parsed = JSON.parse(await llm.generate(prompt)); // no schema — breaks on output drift`,
    },
  },
];
