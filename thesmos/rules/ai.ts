...
export const AI_RULES: ThesmosRule[] = [
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'EU AI Act Annex III classifies credit scoring, hiring, and health risk assessment as high-risk AI. Automated decisions in these domains without a human review gate violate Art. 14 human oversight requirements and can result in fines up to €30M or 6% of global turnover.',
      commonViolations: [
        'const decision = await llm.complete(creditPrompt); await db.updateCreditScore(userId, decision);  // no human gate',
        'const hiringScore = await ai.evaluate(resume); if (hiringScore > 0.8) await hire(candidate);  // fully automated',
      ],
      goodExample: 'const recommendation = await llm.complete(creditPrompt);\nawait queue.push({ type: "credit_decision", payload: recommendation, requiresApproval: true });\n// Human reviewer approves before action is taken',
      badExample: 'const score = await ai.creditRisk(applicant);\nif (score < 0.5) await rejectLoan(applicant.id);  // LLM drives hard decision without oversight',
      relatedPlaybooks: ['ai-governance.md', 'eu-ai-act.md'],
      relatedAgents: ['ai-reviewer', 'compliance-reviewer'],
      relatedSkills: [],
    },
    detect({ config, changedFiles = [] }: DetectInput): Finding[] {
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'EU AI Act Art. 50 requires disclosure when users interact with AI-generated content. Hiding AI origin erodes trust, violates regulations, and can mislead users into false assumptions about authorship.',
      commonViolations: [
        'const reply = await openai.chat(messages); return res.json({ message: reply });  // no disclosure',
        '<div>{aiResponse}</div>  // rendered to user without "AI-generated" label',
      ],
      goodExample: 'const reply = await openai.chat(messages);\nreturn res.json({ message: reply, generatedBy: "AI", disclaimer: "This response was generated by AI and may contain errors." });',
      badExample: 'const advice = await llm.complete(healthPrompt);\nreturn <p>{advice}</p>;  // user does not know this is AI-generated health advice',
      relatedPlaybooks: ['ai-governance.md'],
      relatedAgents: ['ai-reviewer', 'compliance-reviewer'],
      relatedSkills: [],
    },
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'EU AI Act Art. 12 and HIPAA §164.312 require immutable records of AI system actions for post-hoc audits. Without an append-only audit log, you cannot reconstruct what the AI decided, when, or why — making incident response and compliance audits impossible.',
      commonViolations: [
        'const result = await llm.complete(prompt); await db.save(result);  // no audit trail',
        'return ai.classify(input);  // decision returned with no log write',
      ],
      goodExample: 'const result = await llm.complete(prompt);\nawait auditLog.append({ timestamp: Date.now(), input: sanitized, output: result, model, requestId });\nreturn result;',
      badExample: 'const score = await ai.riskScore(patient);\nreturn { approved: score > 0.7 };  // no log of what the model decided or why',
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'EU AI Act Art. 9 and 10 require high-risk AI systems to be tested for bias before deployment. Undocumented classification models can encode historical discrimination — disparate impact in hiring or credit decisions carries significant legal and reputational risk.',
      commonViolations: [
        'const category = await llm.classify(applicant);  // no fairness evaluation before shipping',
        'const score = model.predict(features); return score > threshold;  // no disparate impact check',
      ],
      goodExample: 'const result = await model.classify(input);\n// biasAudit.evaluate() run in CI against balanced test set\nreturn { category: result, biasAuditId: "2024-Q4-audit-001" };',
      badExample: 'const hired = await llm.score(resume) > 0.8;\n// No fairness report, no disparate impact test',
      relatedPlaybooks: ['ai-governance.md'],
      relatedAgents: ['ai-reviewer'],
      relatedSkills: [],
    },
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'Sending PII to a third-party LLM API without a signed DPA violates GDPR Art. 28 and HIPAA BAA requirements. The LLM provider becomes a data processor — without a contract defining their obligations, you have no legal basis for the transfer.',
      commonViolations: [
        'const result = await openai.complete(`Analyze ${user.email} ${user.ssn}`);  // PII to external API, no DPA',
        'llm.chat([{ role: "user", content: `Patient: ${patient.fullName}, DOB: ${patient.dateOfBirth}` }])',
      ],
      goodExample: '// config.dpa = "openai-dpa-2024.pdf" — DPA signed and referenced\nconst anonymized = redact(userData, ["email", "ssn"]);\nconst result = await llm.complete(buildPrompt(anonymized));',
      badExample: 'const insight = await anthropic.complete(`User ${req.body.firstName} ${req.body.email} wants...`);',
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'EU AI Act Art. 13 requires high-risk AI systems to provide sufficient information for users to interpret outputs. Opaque score/rank/classify outputs cannot be appealed or explained to affected parties, creating audit gaps and undermining user rights.',
      commonViolations: [
        'return { score: await model.score(applicant) };  // no reasoning field',
        'const ranking = await llm.rank(candidates); return ranking;  // no explanation of criteria',
      ],
      goodExample: 'const { score, factors } = await model.score(applicant);\nreturn { score, explanation: factors.map(f => f.description), auditId: uuid() };',
      badExample: 'return { approved: aiScore > 0.7 };  // user has no way to understand or contest the decision',
      relatedPlaybooks: ['ai-governance.md'],
      relatedAgents: ['ai-reviewer'],
      relatedSkills: [],
    },
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'GDPR Art. 5(1)(b) requires that personal data is collected for specified purposes and not processed further in incompatible ways. Fine-tuning on user data without consent records means you cannot demonstrate lawful basis — making the trained model itself potentially unlawful.',
      commonViolations: [
        'await openai.createFineTuningJob({ training_file: userDataExport });  // no consent reference',
        'await uploadTrainingFile(db.exportAllUsers());  // no lineage record',
      ],
      goodExample: 'const consentRecord = await consent.verify(userIds, "model-training-v2");\nawait openai.createFineTuningJob({ training_file: fileId, metadata: { consentId: consentRecord.id, dataOrigin: "user-opt-in-2024" } });',
      badExample: 'const file = await openai.files.create({ file: fs.createReadStream("users.jsonl"), purpose: "fine-tune" });\nawait openai.fineTuning.jobs.create({ training_file: file.id, model: "gpt-4o" });',
  ...
  },
  {
    ...
    sinceVersion: '2.1.0',
    explain: {
      why: 'LLM output format is not guaranteed — model upgrades, prompt changes, and temperature variance can silently alter the shape of structured responses. Without a version field, downstream consumers cannot detect schema drift and may silently process malformed data.',
      commonViolations: [
        'const { summary, tags } = await llm.complete(prompt);  // schema not versioned',
        'const z = z.object({ score: z.number(), reason: z.string() });  // no schemaVersion',
      ],
      goodExample: 'const OutputSchema = z.object({ schemaVersion: z.literal("v2"), score: z.number(), reason: z.string() });\nconst result = OutputSchema.parse(await llm.complete(prompt));',
      badExample: 'const result = await llm.complete(classifyPrompt);\nreturn { category: result.category, confidence: result.confidence };  // no version, breaks silently on model update',
      relatedPlaybooks: ['ai-governance.md'],
      relatedAgents: ['ai-reviewer'],
      relatedSkills: [],
    },
