...
function makeEntry(overrides: Partial<PackEntry> = {}): PackEntry {
  return {
    dir: '/tmp/thesmos/packs/web',
    relDir: '.thesmos/packs/web',
...
const EMPTY_DIR = '/tmp/nonexistent-pack-dir-thesmos-test';
...
describe('parsePackManifest', () => {
  it('parses valid JSON manifest', () => {
    ...
    expect(parsed!.id).toBe('@thesmos/web');
  });
...
describe('validatePack', () => {
  ...
  it('valid scoped IDs: @thesmos/core, @company/internal', () => {
    for (const id of ['@thesmos/core', '@company/internal', '@proclip/web-builder']) {
  ...
  it('invalid IDs with uppercase or spaces', () => {
    for (const id of ['@Thesmos/Core', '@foo/Bar Baz', 'thesmos/core']) {
      const manifest = makeManifest({ id });
      const result = validatePack(EMPTY_DIR, manifest);
...
describe('formatPackListConsole', () => {
  ...
  it('shows pack ID and version', () => {
    const out = formatPackListConsole([makeEntry()], 'MyRepo');
    expect(out).toContain('@thesmos/web');
  ...
  it('does not throw when manifest is missing provides or tags', () => {
    ...
    const entry: PackEntry = { dir: '/tmp', relDir: '.thesmos/packs/x', manifest: badManifest, source: 'local' };
...
describe('formatPackListJson', () => {
  ...
  it('includes id, version, source', () => {
    ...
    expect(arr[0].id).toBe('@thesmos/web');
