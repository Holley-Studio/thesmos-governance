...
const SIDEBAR_AGENT_TARGETS = [
  resolve(__dirname, '../../extensions/vscode/src/generated/pantheon-sidebar.ts'),
]

const DIVISIONS_PATH = resolve(__dirname, '../catalog/divisions.json')
 ...
 */
function loadDivisionLabels(): Map<string, string> {
  const map = new Map<string, string>()
  if (!existsSync(DIVISIONS_PATH)) return map
  const divisions = JSON.parse(readFileSync(DIVISIONS_PATH, 'utf-8')) as Array<{
    label: string
    agents: string[]
  }>
  for (const division of divisions) {
    for (const agentId of division.agents) {
      if (!map.has(agentId)) map.set(agentId, division.label)
    }
  }
  return map
}

function displayGodName(agent: AgentMeta): string {
  if (agent.god) return agent.god
  const fromTitle = agent.name.match(/^God Agent\s+(\S+)/i)?.[1]
  if (fromTitle) return fromTitle
  const beforeDash = agent.name.split('—')[0]?.trim()
  if (beforeDash && beforeDash.length <= 24) return beforeDash
  const stem = agent.id.replace(/-agent$/, '').split('-')[0] ?? agent.id
  return stem.charAt(0).toUpperCase() + stem.slice(1)
}

function exportSidebarAgents(agents: AgentMeta[]): { exported: number; skipped: number } {
  const divisionLabels = loadDivisionLabels()
  const entries = agents
    .filter((a) => a.id && a.enabled)
    .sort((a, b) => {
      const domainCmp = (divisionLabels.get(a.id) ?? 'ZZZ').localeCompare(divisionLabels.get(b.id) ?? 'ZZZ')
      if (domainCmp !== 0) return domainCmp
      return displayGodName(a).localeCompare(displayGodName(b))
    })
    .map((a) => {
      const domain = divisionLabels.get(a.id) ?? 'Specialists'
      const name = displayGodName(a)
      const role = a.role || a.name.replace(/^God Agent\s+\S+\s*[—-]\s*/i, '')
      return `  { id: ${JSON.stringify(a.id)}, name: ${JSON.stringify(name)}, domain: ${JSON.stringify(domain)}, role: ${JSON.stringify(role)} },`
    })
    .join('\n')
...
`
  ...
  for (const target of SIDEBAR_AGENT_TARGETS) {
    ensureDir(dirname(target))
    writeFileSync(target, content, 'utf-8')
  }
  return { exported: SIDEBAR_AGENT_TARGETS.length, skipped: 0 }
}
