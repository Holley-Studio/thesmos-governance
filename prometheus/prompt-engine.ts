/**
 * Prometheus Prompt Engine — template interpolation and trigger evaluation.
 *
 ...
 * Templates stored in: prometheus/prompts/ (built-ins) + .prometheus/prompts/ (user)
 */
...
export function loadAllTemplates(root: string): PromptTemplate[] {
  ...
  // User catalog: .prometheus/prompts/
  const userDir = join(root, '.prometheus', 'prompts');
  const userTemplates = loadTemplatesFromDir(userDir, 'user');

  // User-imported catalog: .prometheus/prompts/imported/
  const importedDir = join(root, '.prometheus', 'prompts', 'imported');
