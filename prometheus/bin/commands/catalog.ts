/**
 * prometheus catalog:list      — list all agents and skills
 * prometheus catalog:validate  — validate built-in + user catalog frontmatter
 * prometheus catalog:enable    — add an agent/skill ID to .prometheus/registry.json
 * prometheus catalog:disable   — remove an agent/skill ID from .prometheus/registry.json
 */
...
function readRegistry(root: string): Record<string, unknown> {
  ...
  try {
    ...
    return { rules: ['@prometheus/core'], agents: [], skills: [] };
  }
}
...
export async function cmdCatalog(argv: string[]): Promise<void> {
  ...
  if (sub === 'list') {
    ...
    console.log('Prometheus Catalog');
    ...
    if (user.agents.length > 0 || user.skills.length > 0) {
      printEntries(user.agents, 'User Agents (.prometheus/agents/)', json);
      printEntries(user.skills, 'User Skills (.prometheus/skills/)', json);
    }
    return;
  }
  ...
  if (sub === 'enable' || sub === 'disable') {
    ...
    if (!id) {
      process.stderr.write(`catalog:${sub}: missing <id>\nUsage: prometheus catalog:${sub} <id> [agent|skill]\n`);
      process.exit(1);
    }
    ...
    if (sub === 'enable') {
      if (!list.includes(id)) {
        ...
        console.log(`catalog:enable — added ${type} "${id}" to .prometheus/registry.json`);
      } else {
      ...
      if (updated.length < list.length) {
        ...
        writeRegistry(root, registry);
        console.log(`catalog:disable — removed ${type} "${id}" from .prometheus/registry.json`);
      } else {
