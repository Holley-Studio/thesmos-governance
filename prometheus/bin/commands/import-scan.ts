/**
 * prometheus import:scan — live registry validation for AI-generated imports.
 *
 ...
 *
 * Usage:
 *   prometheus import:scan                  # Scan all JS/TS/Python files
 *   prometheus import:scan src/api/auth.ts  # Scan specific files
 *   prometheus import:scan --strict          # Also flag packages < 30 days old
 *   prometheus import:scan --ci              # Exit 1 on any BLOCKER finding
 *   prometheus import:scan --json            # Machine-readable output
 */
...
export async function cmdImportScan(argv: string[]): Promise<void> {
  ...
  if (!asJson) {
    console.log(`\nPrometheus Import Scan${strict ? ' (strict mode)' : ''}\n`);
