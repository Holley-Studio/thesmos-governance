/**
 * prometheus baseline:create / baseline:update / baseline:report
 *
...
export async function cmdBaseline(subcommand: string, argv: string[]): Promise<void> {
  ...
  if (!scan) {
    process.stderr.write(
      `prometheus baseline:${subcommand}: .prometheus/report.json not found — run prometheus scan first\n`
    );
    process.exit(1);
  }
  ...
  if (subcommand === 'create') {
    ...
    if (existing && !flag(flags, 'force')) {
      process.stderr.write(
        `prometheus baseline:create: baseline already exists at ${BASELINE_PATH}\n` +
    ...
    process.stdout.write(
      `✅  Baseline created at ${BASELINE_PATH}\n` +
        ...
        `   Future runs of prometheus validate will only fail on NEW violations.\n`
    );
    return;
  }
  ...
  if (subcommand === 'update') {
    ...
    if (!existing) {
      process.stderr.write(
        `prometheus baseline:update: no baseline found — run baseline:create first\n`
      );
      process.exit(1);
    }
    ...
    process.stdout.write(`Prometheus Baseline — ${config.project}\n`);
  ...
  if (subcommand === 'report') {
    ...
    if (!existing) {
      process.stderr.write(
        `prometheus baseline:report: no baseline found — run baseline:create first\n`
      );
      process.exit(1);
    }
  ...
  process.stderr.write(
    `prometheus baseline: unknown subcommand "${subcommand}"\n` +
