/**
 * prometheus scan — walk the repo, detect stack, write .prometheus/report.json.
...
export function formatScanConsole(scan: ScanResult, config: PrometheusConfig): string {
  ...
  const lines: string[] = [
    `Prometheus Scan — ${config.project}`,
    '',
  ];
...
export function formatScanMarkdown(scan: ScanResult, config: PrometheusConfig): string {
  const d = scan.detector;
  const lines = [`## Prometheus Scan — ${config.project}\n`];
...
export async function cmdScan(argv: string[]): Promise<void> {
  ...
  // Persist to .prometheus/report.json, preserving any manually-added keys
