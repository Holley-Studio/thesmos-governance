...
export async function cmdAudit(argv: string[]): Promise<void> {
  ...
  if (!scan) {
    process.stderr.write('prometheus audit: .prometheus/report.json not found — run prometheus scan first\n');
    process.exit(1);
  }
  ...
  if (markdown) {
    const lines = [
      `## Prometheus Audit — ${config.project}\n`,
      '### Installation',
