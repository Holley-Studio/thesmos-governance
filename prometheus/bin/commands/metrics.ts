 ...
 *   --record          append a snapshot to .prometheus/metrics-history.jsonl
...
function formatHistoryConsole(history: MetricsSnapshot[], projectName: string): string {
  const lines: string[] = [''];
  const SEP = '─'.repeat(72);
  lines.push(`  Prometheus Metrics History — ${projectName}`);
  lines.push(SEP);
  if (history.length === 0) {
    lines.push('  No snapshots yet. Run: prometheus metrics --record');
    lines.push('');
    return lines.join('\n');
  }
  lines.push('');
...
export async function cmdMetrics(argv: string[]): Promise<void> {
  ...
  if (record) {
    ...
    process.stderr.write(`Snapshot recorded to .prometheus/metrics-history.jsonl\n`);
  }
