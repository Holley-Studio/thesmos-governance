/**
 * prometheus update — convenience command.
 *
 * Runs in sequence:
 *   1. scan        — refresh .prometheus/report.json
...
export async function cmdUpdate(argv: string[]): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`prometheus update: scan failed — ${err instanceof Error ? err.message : err}\n`);
    process.exit(1);
  }
  ...
  if (!skipAdapters) {
    ...
    try {
      ...
      process.stderr.write(`prometheus update: adapters failed — ${err instanceof Error ? err.message : err}\n`);
      process.exit(1);
    }
  } else {
  ...
  try {
    ...
    process.stderr.write(`prometheus update: drift check failed — ${err instanceof Error ? err.message : err}\n`);
    process.exit(1);
  }
