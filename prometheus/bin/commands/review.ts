/**
 * prometheus review — run review checks and report findings.
...
export async function cmdReview(argv: string[]): Promise<void> {
  ...
  if (!scan) {
    process.stderr.write('prometheus review: .prometheus/report.json not found — run prometheus scan first\n');
    process.exit(1);
  }
