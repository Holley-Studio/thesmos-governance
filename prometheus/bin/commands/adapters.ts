/**
 * prometheus adapters — generate AI adapter files from canonical Prometheus rules.
...
export async function cmdAdapters(argv: string[]): Promise<void> {
  ...
  if (targets.length === 0) {
    process.stderr.write('prometheus adapters: no valid targets specified\n');
    process.exit(1);
  }
  ...
  if (markdown) {
    const lines = [`## Prometheus Adapters — ${config.project}\n`];
  ...
  console.log(`Prometheus Adapters — ${config.project}`);
