/**
 * prometheus suppressions:audit
 *
 * Scans the project for prometheus-disable-next-line comments and audits them for:
 *   - missing reasons
...
export async function cmdSuppressions(subcommand: string, argv: string[]): Promise<void> {
  ...
  if (subcommand !== 'audit') {
    process.stderr.write(
      `prometheus suppressions: unknown subcommand "${subcommand}"\n` +
  ...
  if (!scan) {
    process.stderr.write(
      'prometheus suppressions:audit: .prometheus/report.json not found — unused-suppression check skipped\n'
    );
  }
