/**
 * prometheus ci — single-command CI gate.
 *
 ...
 *   - run: npx prometheus ci --base=main
 */
...
export async function cmdCiGate(argv: string[]): Promise<void> {
  ...
  if (!scan) {
    process.stderr.write(
      'prometheus ci: .prometheus/report.json not found — run prometheus scan first\n'
    );
    process.exit(1);
  }
  ...
  const lines: string[] = [];
  lines.push('');
  lines.push(`  Prometheus CI — ${config.project}`);
  lines.push(SEP);
  lines.push('');
