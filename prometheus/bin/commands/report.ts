...
export function formatFindingsHtml(
  ...
  <div class="section">
    ...
    <div class="pack-empty">No packs installed — run <code>prometheus pack:create @org/name</code> to author one.</div>
  </div>`;
    }
  ...
  <div class="section">
    ...
    <p class="trend-empty">Not enough data — run <code>prometheus metrics --record</code> to start tracking.</p>
  </div>`;
    }
...
export async function cmdReport(argv: string[]): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`[prometheus] Could not write report to ${outPath}: ${msg}\n`);
    process.exit(1);
  }
