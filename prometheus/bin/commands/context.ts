/**
 * prometheus context:snapshot  — generate .prometheus/context.md
...
export async function cmdContext(argv: string[]): Promise<void> {
  ...
  process.stderr.write(`prometheus context: unknown subcommand "${sub}"\n`);
  process.stderr.write('Usage: prometheus context:snapshot | prometheus context:health\n');
  process.exit(1);
}
