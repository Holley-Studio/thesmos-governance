/**
 * prometheus validate — review checks that exit 1 on failOnSeverity findings.
...
export async function cmdValidate(argv: string[]): Promise<void> {
  ...
  if (!scan) {
    process.stderr.write('prometheus validate: .prometheus/report.json not found — run prometheus scan first\n');
    process.exit(1);
  }
  ...
  if (baseline) {
    const suppressed = allFindings.length - findings.length;
    if (suppressed > 0) {
      process.stderr.write(`\nnote: ${suppressed} baseline finding${suppressed === 1 ? '' : 's'} suppressed — run prometheus baseline:report for details\n`);
    }
  }
