/**
 * prometheus explain — make every rule self-documenting.
 *
 * Usage:
 *   prometheus explain <rule-id|category>     explain a specific rule
 *   prometheus explain file <path>            explain all rules active on a file
 *   prometheus explain finding <fingerprint>  explain the rule for a finding
 *   prometheus explain --list                 list all rules
 *
 * Flags:
...
import type { PrometheusRule } from '../../types.ts';
...
export async function cmdExplain(argv: string[]): Promise<void> {
  ...
  // prometheus explain file <path>
  if (subOrTarget === 'file') {
    const filePath = rest[0];
    if (!filePath) {
      process.stderr.write('prometheus explain file: path required\n');
      process.exit(1);
    }
  ...
  // prometheus explain finding <fingerprint>
  if (subOrTarget === 'finding') {
    const fp = rest[0];
    if (!fp) {
      process.stderr.write('prometheus explain finding: fingerprint prefix required\n');
      process.exit(1);
    }
    const findings = loadAndReview(root, config);
    if (!findings) return;
  ...
  // prometheus explain <rule-id|category>
  const rule = findRule(subOrTarget);
  if (!rule) {
    process.stderr.write(
      `prometheus explain: unknown rule or category "${subOrTarget}"\n` +
        `  Run: prometheus explain --list  to see all rules\n`
    );
    process.exit(1);
  }
...
function loadAndReview(
  ...
  const scan = loadReport(root);
  if (!scan) {
    process.stderr.write('prometheus explain: .prometheus/report.json not found — run prometheus scan first\n');
    process.exit(1);
  }
...
function outputRules(rules: PrometheusRule[], json: boolean, markdown: boolean, liveViolations?: Finding[]): void {
  if (json) {
    const out = rules.map((r) => ({
