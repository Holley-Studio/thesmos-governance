...
describe('parseSuppression', () => {
  it('parses a complete suppression comment', () => {
    const line = '// prometheus-disable-next-line missing_api_auth -- reason: protected by middleware -- owner: @alice -- expires: 2027-01-01';
  ...
  it('parses minimal suppression (rule + reason only)', () => {
    const line = '// prometheus-disable-next-line missing_api_auth -- reason: gateway auth';
  ...
  it('parses suppression without reason (will trigger audit finding)', () => {
    const line = '// prometheus-disable-next-line missing_api_auth';
  ...
  it('is case-insensitive for the directive', () => {
    const line = '// PROMETHEUS-DISABLE-NEXT-LINE missing_api_auth -- reason: test';
  ...
  it('handles rule ID with no rest text (blanket-style)', () => {
    const line = '// prometheus-disable-next-line';
    const s = parseSuppression(line, 1, FILE);
    expect(s).not.toBeNull();
...
describe('extractSuppressions', () => {
  it('extracts all suppression comments from file content', () => {
    const content = [
      'const x = 1;',
      '// prometheus-disable-next-line console_log -- reason: debug temp',
      'console.log("x:", x);',
      '// prometheus-disable-next-line any_type_no_comment -- reason: external SDK',
...
describe('formatSuppressionAuditMarkdown', () => {
  it('is valid markdown with heading', () => {
    ...
    expect(out).toContain('## Prometheus Suppressions Audit');
    expect(out).toContain('All suppressions are valid');
  });
