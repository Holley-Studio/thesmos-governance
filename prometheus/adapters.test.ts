...
// Claude gets its own suite — it's a thin adapter referencing .prometheus/ files,
...
describe('generateClaudeRules (thin adapter)', () => {
  ...
  it('references .prometheus/governance/CODE_REVIEW.md', () => {
    expect(output).toContain('.prometheus/governance/CODE_REVIEW.md');
  });

  it('references prometheus:validate command', () => {
    expect(output).toContain('prometheus:validate');
  });

  it('references prometheus:review command', () => {
    expect(output).toContain('prometheus:review');
  });
...
describe('Claude adapter preamble (thin adapter contract)', () => {
  ...
  it('references .prometheus/README.md', () => {
    expect(claudeDoc).toContain('.prometheus/README.md');
  });

  it('references .prometheus/GUARDRAILS.md', () => {
    expect(claudeDoc).toContain('.prometheus/GUARDRAILS.md');
  });

  it('references .prometheus/report.json', () => {
    expect(claudeDoc).toContain('.prometheus/report.json');
  });

  it('references .prometheus/governance/CODE_REVIEW.md', () => {
    expect(claudeDoc).toContain('.prometheus/governance/CODE_REVIEW.md');
  });

  it('references .prometheus/governance/REVIEW_AGENT.md', () => {
    expect(claudeDoc).toContain('.prometheus/governance/REVIEW_AGENT.md');
  });

  it('references .prometheus/playbooks/', () => {
    expect(claudeDoc).toContain('.prometheus/playbooks/');
  });
  ...
  it('instructs to run a Prometheus command after changes', () => {
    expect(claudeDoc).toContain('prometheus:scan');
  });
...
describe('writeAllAdapters', () => {
  ...
  it('writes no files to the library source directory', () => {
    ...
    expect(root).not.toContain('prometheus-helper/prometheus');
  });
  ...
  it('a subset of targets writes only those adapter files', () => {
    ...
    expect(existsSync(join(root, '.cursor/rules/prometheus.mdc'))).toBe(false);
  });
});
