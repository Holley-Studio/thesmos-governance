 ...
 *
 * Reads .prometheus/scope.json (or defaults) and validates every Write/Edit/Bash
 ...
 *
 * Scope config is loaded from .prometheus/scope.json. If no file exists, all
...
const SCOPE_FILE = '.prometheus/scope.json';
  ...
  for (const blocked of config.workspace.blockedPaths) {
    ...
    if (re.test(relPath) || re.test(filePath)) {
      return {
        ...
        suggestion: `Edit .prometheus/scope.json to allow this path if needed, or handle this file outside the agent session.`,
      };
    }
  }
  ...
  if (config.workspace.allowedPaths.length > 0) {
    ...
    if (!isAllowed) {
      return {
        ...
        suggestion: `Add "${relPath}" to allowedPaths in .prometheus/scope.json, or restrict the agent's task to files within the allowed scope.`,
      };
    }
  }
...
function checkCommand(command: string, config: ScopeConfig): ScopeViolation | null {
  ...
  for (const pattern of config.destructivePatterns) {
    if (cmd.includes(pattern.toLowerCase())) {
      return {
        ...
        suggestion: 'Prometheus scope enforcement blocked this command. If this is intentional, run it manually outside the agent session.',
      };
    }
  }
    ...
    if (/\brm\s+(?:-[a-z]*f[a-z]*\s+)?\S/.test(cmd) && !/\brm\s+--/.test(cmd)) {
      ...
      if (!isHelp) {
        return {
          ...
          suggestion: 'Set operations.allowDelete to true in .prometheus/scope.json to enable file deletion, or delete manually.',
        };
      }
    }
  }
  ...
  if (!config.operations.allowGitPush && /\bgit\s+push\b/.test(cmd)) {
    return {
      ...
      suggestion: 'Set operations.allowGitPush to true in .prometheus/scope.json, or push manually after reviewing the changes.',
    };
  }
    ...
    for (const re of dbWritePatterns) {
      if (re.test(command)) {
        return {
          ...
          suggestion: 'Set operations.allowDatabaseWrites to true in .prometheus/scope.json, or run database commands manually.',
        };
      }
    }
  }
  ...
  for (const pattern of config.operations.requireConfirmation) {
    if (cmd.includes(pattern.toLowerCase())) {
      return {
        ...
        suggestion: 'Run this command manually after reviewing the agent\'s changes, or add it to an exception in .prometheus/scope.json.',
      };
    }
  }
