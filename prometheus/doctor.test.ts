...
function makeFullInput(overrides: Partial<DoctorInput> = {}): DoctorInput {
  ...
  return {
    ...
    readJsonSafe: (rel) => {
      if (rel === '.prometheus/report.json') return freshReport;
      if (rel === '.prometheus/config.json') return validConfigJson;
      return null;
    },
    packageScripts: Object.fromEntries(
      CONFIG_DEFAULTS.doctor.requiredScripts.map((s) => [s, `prometheus ${s.replace('prometheus:', '')}`])
    ),
    now: new Date(),
    ...overrides,
  };
...
function makeEmptyInput(configOverride?: Partial<PrometheusConfig>): DoctorInput {
  return {
...
describe('package scripts check', () => {
  ...
  it('fails for a specific missing script', () => {
    const scripts = Object.fromEntries(
      CONFIG_DEFAULTS.doctor.requiredScripts
        .filter((s) => s !== 'prometheus:doctor')
        .map((s) => [s, `prometheus ${s.replace('prometheus:', '')}`])
    );
    const input = makeFullInput({ packageScripts: scripts });
    const checks = runDoctor(input);
    const doctorScript = checks.find((c) => c.name === 'script:prometheus:doctor');
  ...
  it('fix hint mentions the script name', () => {
    const checks = runDoctor(makeEmptyInput());
    const scriptCheck = checks.find((c) => c.name === 'script:prometheus:scan');
    expect(scriptCheck?.fixHint).toContain('prometheus:scan');
  });
});
...
describe('adapter files check', () => {
  ...
  it('each failing check includes a fix hint', () => {
    ...
    for (const c of adapterChecks) {
      expect(c.fixHint).toContain('prometheus adapters');
    }
  });
...
describe('report health — missing report', () => {
  it('fails with a single check when report.json does not exist', () => {
    ...
    expect(reportChecks[0].fixHint).toContain('prometheus scan');
  });
});
...
describe('report health — stale report', () => {
  it('fails freshness check when report is older than maxAgeDays', () => {
    ...
    const input = makeFullInput({
      readJsonSafe: (rel) => {
        if (rel === '.prometheus/report.json') {
          return { generatedAt: staleDate.toISOString(), _generatedSections: [] };
        }
        if (rel === '.prometheus/config.json') return { name: 'Test', version: '2.0.0' };
        return null;
      },
    });
    ...
    expect(freshCheck?.fixHint).toContain('prometheus scan');
  ...
  it('fails freshness when generatedAt is missing', () => {
    const input = makeFullInput({
      readJsonSafe: (rel) => {
        if (rel === '.prometheus/report.json') return { _generatedSections: [] };
        if (rel === '.prometheus/config.json') return { name: 'Test', version: '2.0.0' };
        return null;
      },
    });
...
describe('configuration — config.json', () => {
  ...
  it('fails when config.json is missing required fields', () => {
    const input = makeFullInput({
      readJsonSafe: (rel) => {
        if (rel === '.prometheus/config.json') return { project: 'test' }; // missing name+version
        if (rel === '.prometheus/report.json')
...
describe('IDE integration checks', () => {
  ...
  it('fix hint for .cursor mentions prometheus adapters', () => {
    const checks = runDoctor(makeEmptyInput());
    const cursorCheck = checks.find((c) => c.name === 'ide:.cursor');
    expect(cursorCheck?.fixHint).toContain('prometheus adapters');
  });
  ...
  it('adds an informational secrets reminder when requiresSecrets is set and workflow exists', () => {
    const input = makeFullInput({
      config: {
        ...CONFIG_DEFAULTS,
        github: {
          workflow: '.github/workflows/prometheus-review.yml',
...
describe('formatDoctorConsole', () => {
  ...
  it('groups checks by category', () => {
    const checks = runDoctor(makeFullInput());
    const out = formatDoctorConsole(checks);
    expect(out).toContain('Prometheus files');
...
describe('formatDoctorMarkdown', () => {
  it('includes a markdown heading', () => {
    ...
    expect(md).toContain('## Prometheus Doctor');
