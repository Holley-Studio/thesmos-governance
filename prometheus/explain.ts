...
import type { PrometheusRule, RuleExplanation } from './types.js';
...
export function findRule(idOrCategory: string): PrometheusRule | null {
...
export function findRulesForFile(file: string, findings: Finding[]): PrometheusRule[] {
 ...
 */
export function findRuleForFingerprint(prefix: string, findings: Finding[]): PrometheusRule | null {
...
export function listRules(): PrometheusRule[] {
...
export function formatExplainConsole(rule: PrometheusRule): string {
  const lines: string[] = [];
  const SEP = '─'.repeat(60);
...
export function formatExplainMarkdown(rule: PrometheusRule): string {
  const lines: string[] = [];
  const ex = explainSection(rule.explain);
...
export function formatExplainJson(rule: PrometheusRule): string {
...
export function formatRuleListLine(rule: PrometheusRule): string {
  return `  ${SEVERITY_EMOJI[rule.severity]}  ${rule.id.padEnd(10)}  ${rule.category.padEnd(30)}  ${rule.severity}`;
}

export function formatExplainListConsole(rules: PrometheusRule[]): string {
  const lines: string[] = [];
  lines.push('');
