/**
 * Prometheus Brain Learn — BYOK Claude API integration.
 *
...
function buildLearnPrompt(data: LearnPromptData): string {
  return JSON.stringify({
    task: 'prometheus-governance-analysis',
    instructions: [
      'Analyze the governance data from a Prometheus repository scan.',
...
export async function learnFromBrainStore(
  ...
  // Pre-flight report
  console.log('\n  prometheus brain:learn — pre-flight check\n');
...
export function formatDisabledError(): string {
  return [
    '',
    '  prometheus brain:learn: disabled',
    '',
    '  brain:learn is disabled in .prometheus/config.json (brain.learn.enabled=false).',
    '  To re-enable: prometheus brain:enable',
    '',
    '  Note: All other Prometheus features work without an API key.',
    '',
  ].join('\n');
}
