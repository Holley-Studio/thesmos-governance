/**
 * Prometheus doctor command.
 * Verifies that Prometheus is correctly installed and usable in any repo.
 *
...
import type { DoctorCheck, PrometheusConfig } from './types';
...
/** Check that all required Prometheus governance files are present. */
function checkRequiredFiles(input: DoctorInput): DoctorCheck[] {
  return input.config.doctor.requiredFiles.map((relPath) => {
    ...
    return {
      ...
      fixHint: pass ? undefined : 'Run prometheus init to create missing Prometheus files',
    };
  });
}
...
function checkPackageScripts(input: DoctorInput): DoctorCheck[] {
  return input.config.doctor.requiredScripts.map((scriptName) => {
    ...
    const cmd = scriptName.replace('prometheus:', '');
    return {
      ...
      fixHint: pass
        ? undefined
        : `Add "${scriptName}": "prometheus ${cmd}" to your package.json scripts block`,
    };
  });
}
  ...
  return Object.entries(ADAPTER_OUTPUT_PATHS).map(([target, relPath]) => {
    ...
    if (!exists) {
      return {
        ...
        fixHint: 'Run prometheus adapters to generate all AI adapter files',
      };
    }
    ...
    if (input.readFileSafe) {
      ...
      if (content !== null) {
        ...
        return {
          ...
          fixHint: fresh ? undefined : 'Run prometheus adapters to refresh AI adapter files',
        };
      }
    }
    return {
...
function checkReportHealth(input: DoctorInput): DoctorCheck[] {
  ...
  const existsCheck: DoctorCheck = {
    ...
    fixHint: exists ? undefined : 'Run prometheus scan to generate the initial scan report',
  };
  ...
  const freshCheck: DoctorCheck = {
    ...
    fixHint: stale ? 'Run prometheus scan to refresh the report' : undefined,
  };
...
function checkConfiguration(input: DoctorInput): DoctorCheck[] {
  const configPath = '.prometheus/config.json';
  ...
  const checks: DoctorCheck[] = [
    {
      ...
      fixHint: configValid
        ? undefined
        : 'Run prometheus init to create a valid config.json, or ensure it contains "name" and "version" string fields',
    },
  ];
  ...
  checks.push({
    ...
    fixHint: hasBranches
      ? undefined
      : 'Add "protectedBranches": ["main"] to .prometheus/config.json',
  });
...
const IDE_FIX_HINTS: Record<string, string> = {
  '.claude':
    'Create a .claude/ directory; run prometheus adapters to generate CLAUDE.md',
  '.cursor':
    'Create a .cursor/rules/ directory; run prometheus adapters to generate prometheus.mdc',
  '.codex':
    'Create a .codex/ directory; run prometheus adapters to generate prometheus.md',
};
...
function checkGitHubIntegration(input: DoctorInput): DoctorCheck[] {
  const workflowPath =
    input.config.github?.workflow ?? '.github/workflows/prometheus-review.yml';
  ...
  const checks: DoctorCheck[] = [
    {
      ...
      fixHint: pass
        ? undefined
        : 'Create the GitHub Actions workflow file or run prometheus init --github',
    },
  ];
...
export function runDoctorForRoot(root: string, config: PrometheusConfig): DoctorCheck[] {
