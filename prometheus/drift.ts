/**
 * Prometheus Drift Detection — proves that governance is actually in sync.
 *
...
const GOVERNANCE_SECTIONS: Array<{ path: string; sectionId: string }> = [
  { path: '.prometheus/GUARDRAILS.md', sectionId: 'rules' },
  { path: '.prometheus/RULES.md', sectionId: 'rules' },
  { path: '.prometheus/governance/CODE_REVIEW.md', sectionId: 'checklist' },
  { path: '.prometheus/governance/REVIEW_AGENT.md', sectionId: 'instructions' },
];

// ── Helpers ───────────────────────────────────────────────────────────────────

/**
 * Remove the "## Active Prometheus Context" catalog context block that
  ...
  for (const [target, relPath] of Object.entries(ADAPTER_OUTPUT_PATHS)) {
    if (!input.fileExists(relPath)) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus adapters to generate all AI adapter files',
      });
    }
  }
  ...
  for (const { path: relPath } of GOVERNANCE_SECTIONS) {
    if (!input.fileExists(relPath)) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus init to create all governance files',
      });
    }
  }
  ...
  for (const [target, relPath] of Object.entries(ADAPTER_OUTPUT_PATHS)) {
    ...
    if (!meta) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus adapters to regenerate with embedded metadata',
      });
      continue;
    }
    ...
    if (meta.ruleCount !== input.rules.length) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus adapters to regenerate adapter files from the current rule registry',
      });
    }
    ...
    if (meta.version !== input.config.version) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus adapters to refresh adapter files after upgrading Prometheus',
      });
    }
  }
  ...
  for (const [target, relPath] of Object.entries(ADAPTER_OUTPUT_PATHS)) {
    ...
    if (actualCore !== expectedCore) {
      findings.push({
        ...
        fixSuggestion: 'Remove edits inside the PROMETHEUS:GENERATED markers and run prometheus adapters, or move content outside the markers',
      });
    }
  }
  ...
  for (const { path: relPath, sectionId } of GOVERNANCE_SECTIONS) {
    ...
    if (!actualSection) {
      ...
      findings.push({
        ...
        fixSuggestion: 'Run prometheus init to regenerate governance files',
      });
      continue;
    }
    ...
    if (actualSection !== expectedSection) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus init to regenerate governance files from the current rule registry',
      });
    }
  }
...
function checkRegistryPropagation(input: DriftInput): DriftFinding[] {
  ...
  const CATALOG_SECTION = '## Active Prometheus Context';
  ...
  for (const [target, relPath] of Object.entries(ADAPTER_OUTPUT_PATHS)) {
    ...
    if (!content.includes(CATALOG_SECTION)) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus adapters to propagate the registry configuration into all adapter files',
      });
    }
  }
...
function checkRegistryFiles(input: DriftInput): DriftFinding[] {
  ...
  for (const id of input.registryAgentIds) {
    const relPath = `.prometheus/agents/${id}.md`;
    if (!input.fileExists(relPath)) {
      findings.push({
        ...
        fixSuggestion: `Run prometheus agent:create "${id}" or prometheus catalog:enable ${id} agent to install the file`,
      });
    }
  }

  for (const id of input.registrySkillIds) {
    const relPath = `.prometheus/skills/${id}.md`;
    if (!input.fileExists(relPath)) {
      findings.push({
        ...
        fixSuggestion: `Run prometheus skill:create "${id}" or prometheus catalog:enable ${id} skill to install the file`,
      });
    }
  }
  ...
  for (const id of input.registryAgentIds) {
    if (!input.knownAgentIds.has(id)) {
      findings.push({
        ...
        severity: 'MEDIUM',
        file: `.prometheus/agents/${id}.md`,
        message: `Agent "${id}" is in registry.json but not found in any catalog (built-in or user)`,
        fixSuggestion: `Run prometheus agent:create "${id}" to scaffold the agent file, then add it to the catalog`,
      });
    }
  }
  ...
  for (const id of input.registrySkillIds) {
    if (!input.knownSkillIds.has(id)) {
      findings.push({
        ...
        severity: 'MEDIUM',
        file: `.prometheus/skills/${id}.md`,
        message: `Skill "${id}" is in registry.json but not found in any catalog (built-in or user)`,
        fixSuggestion: `Run prometheus skill:create "${id}" to scaffold the skill file, then add it to the catalog`,
      });
    }
  }
  ...
  for (const profileId of input.registryProfileIds) {
    ...
    if (!expected) {
      findings.push({
        ...
        fixSuggestion: 'Run prometheus catalog:profiles to list available profiles',
      });
      continue;
    }

    for (const agentId of expected.agents) {
      const relPath = `.prometheus/agents/${agentId}.md`;
      if (!input.fileExists(relPath)) {
        findings.push({
          ...
          fixSuggestion: `Run prometheus init --profile=${profileId} to install all profile artifacts`,
        });
      }
    }

    for (const skillId of expected.skills) {
      const relPath = `.prometheus/skills/${skillId}.md`;
      if (!input.fileExists(relPath)) {
        findings.push({
          type: 'profile.missing-artifact',
          severity: 'MEDIUM',
...
function checkReportStaleness(input: DriftInput): DriftFinding[] {
  const findings: DriftFinding[] = [];
  const reportPath = '.prometheus/report.json';
  ...
  if (!input.fileExists(reportPath)) {
    findings.push({
      ...
      severity: 'MEDIUM',
      file: reportPath,
      message: 'report.json is missing — prometheus scan has not been run',
      fixSuggestion: 'Run prometheus scan to generate the initial repo intelligence report',
    });
    return findings;
  }
  ...
  if (isReportStale(generatedAt, maxDays, input.now.getTime())) {
    ...
    findings.push({
      ...
      fixSuggestion: 'Run prometheus scan to refresh the repo intelligence report',
    });
  }
...
export function runDriftForRoot(root: string, config: PrometheusConfig): DriftFinding[] {
  // Load registry
...
export function formatDriftConsole(
  ...
  const lines: string[] = [];
  lines.push(`Prometheus Drift — ${projectName}`);
