/**
 * Prometheus Brain — context that survives Claude Code compaction.
 *
 * Generates `.prometheus/brain.md` — a concise Markdown file optimized for
 ...
 *   - `prometheus brain:snapshot`  — full snapshot
 *   - `prometheus brain:compact`   — minimal (<500 words) for pre-compaction
...
export const BRAIN_FILE = '.prometheus/brain.md';
export const BRAIN_DIR = '.prometheus';
...
function readSuppressionsFile(root: string): ActiveSuppression[] {
  const suppressPath = join(root, '.prometheus', 'suppressions.json');
...
function readArchitectureNotes(root: string): string[] {
  ...
  const capsulePath = join(root, '.prometheus', 'context.md');
...
function readCustomRules(root: string): string[] {
  const customDir = join(root, '.prometheus', 'custom-rules');
...
function formatFullBrain(s: BrainSnapshot): string {
  ...
  const lines: string[] = [
    '---',
    'prometheus-brain: true',
    `generated: ${s.generatedAt}`,
    `health: ${health}`,
    '---',
    '',
    `# Prometheus Brain — ${s.projectName}`,
    '',
  ...
  lines.push('```');
  lines.push('prometheus scan              # Re-scan the repo');
  lines.push('prometheus self:check        # Check Prometheus health');
  lines.push('prometheus brain:snapshot    # Refresh this brain file');
  lines.push('prometheus explain <rule>    # Explain any finding');
  lines.push('```');
  lines.push('');
...
export function injectBrainImportIntoCLAUDEMD(root: string): boolean {
  ...
  if (content.includes('prometheus-brain-import')) return false; // already injected

  const injection = [
    '',
    '## Prometheus Governance Brain',
    '<!-- prometheus-brain-import -->',
    '@.prometheus/brain.md',
    '<!-- /prometheus-brain-import -->',
    '',
  ].join('\n');
...
export function installBrainStopHook(root: string): boolean {
  ...
  const hookCmd = 'npx prometheus brain:compact 2>/dev/null || true';
