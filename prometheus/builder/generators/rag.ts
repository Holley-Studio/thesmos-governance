 ...
 *   - prometheus/rag/<name>/chunker.ts
 *   - prometheus/rag/<name>/retriever.ts
 *   - prometheus/rag/<name>/pipeline.ts
 *   - prometheus/mcp-tools/<name>-rag.ts  (if mcpTool === 'yes')
 */
...
function buildRetriever(answers: WizardAnswers): string {
  ...
  const embedComment = embedModel === 'openai'
    ? '// Uses ANTHROPIC_API_KEY or OPENAI_API_KEY — BYOK, never stored by Prometheus'
    : embedModel === 'anthropic'
    ? '// Uses ANTHROPIC_API_KEY — BYOK, never stored by Prometheus'
 ...
 * Purpose: ${job}
 *
 * Register in prometheus/mcp-server.ts TOOL_DEFINITIONS array.
 */
...
export function generateRagPlan(answers: WizardAnswers, context: WizardContext): string {
  ...
  return [
    ...
    `## Files to create`,
    '',
    `- \`prometheus/rag/${name}/chunker.ts\` — document chunking`,
    `- \`prometheus/rag/${name}/retriever.ts\` — vector storage + similarity search`,
    `- \`prometheus/rag/${name}/pipeline.ts\` — ingestion + query pipeline`,
    mcpTool === 'yes' ? `- \`prometheus/mcp-tools/${name}-rag.ts\` — MCP tool wrapper` : '',
    '',
    ...
    mcpTool === 'yes' ? `- [ ] Register \`${name}_rag_query\` in prometheus/mcp-server.ts` : '',
    `- [ ] Run governance scan: prometheus review prometheus/rag/${name}/`,
    '',
    ...
    '',
    `---`,
    `*Generated by prometheus build:rag --plan*`,
    `*Run: prometheus build:rag --scaffold to write code files*`,
  ].filter((l) => l !== '').join('\n');
}
...
export async function generateRag(
  ...
  if (opts.scaffold) {
    files.push({
      path: `prometheus/rag/${name}/chunker.ts`,
      content: buildChunker(answers),
      label: 'Document chunker',
    });
    files.push({
      path: `prometheus/rag/${name}/retriever.ts`,
      content: buildRetriever(answers),
      label: 'Vector retriever',
    });
    files.push({
      path: `prometheus/rag/${name}/pipeline.ts`,
      content: buildPipeline(answers),
      label: 'RAG pipeline',
    });
    if (mcpTool === 'yes') {
      files.push({
        path: `prometheus/mcp-tools/${name}-rag.ts`,
