 ...
 *
 * Outputs:
 *   - .prometheus/catalog/agents/<name>.md  — agent definition + system prompt
 *   - .claude/commands/<name>.md            — Claude Code slash command
 *   - (optional) prometheus/agents/<name>.ts        — TypeScript skeleton
 *   - (optional) prometheus/agents/<name>.test.ts   — test scaffold
 */
...
function buildSystemPrompt(answers: WizardAnswers, context: WizardContext): string {
  ...
  const toolGuidance = toolAccess === 'none'
    ...
    : toolAccess === 'readonly'
    ? 'You may use read-only Prometheus tools (scan_file, explain_rule, get_context). Do not write files.'
  ...
  return [
    ...
    '',
    `## Governance`,
    `This agent runs under Prometheus governance. Every action is subject to:`,
    `- Prometheus security rules (1,075+ rules active)`,
    `- Audit trail logging`,
...
function buildAgentCatalog(answers: WizardAnswers, context: WizardContext, systemPrompt: string): string {
  ...
  return [
    ...
    '```bash',
    `# From terminal:`,
    `npx prometheus agent:run ${name}`,
    `npx prometheus agent:run ${name} --dry-run`,
    '',
    `# From Claude Code:`,
    `/${name}`,
    '```',
    '',
    `## Governance`,
    `- Generated by Prometheus Builder Wizard`,
    `- Governed by ${context.detectedStack.join(', ') || 'default'} rules`,
    `- Audit trail: enabled`,
    '',
    `*Generated by prometheus build:agent*`,
  ].join('\n');
}
...
function buildClaudeCommand(answers: WizardAnswers, systemPrompt: string): string {
  ...
  return [
    ...
    `---`,
    '',
    `*Prometheus-governed agent. Run \`prometheus self:check\` to verify governance is active.*`,
  ].join('\n');
}
...
function buildAgentTypescript(answers: WizardAnswers): string {
  ...
  return [
    ...
    ` *`,
    ` * Generated by prometheus build:agent`,
    ` * Governance: Prometheus 1,075+ rules active`,
    ` */`,
    '',
    `import { makeLogger } from '../logger.js';`,
    '',
...
export async function generateAgent(
  ...
  const files: AgentArtifact['files'] = [
    {
      path: `.prometheus/catalog/agents/${name}.md`,
...
export function generateAgentPlan(answers: WizardAnswers, context: WizardContext): string {
  ...
  return [
    ...
    '',
    `- [ ] Create .prometheus/catalog/agents/${name}.md`,
    `- [ ] Create .claude/commands/${name}.md`,
    `- [ ] Implement core logic in prometheus/agents/${name}.ts`,
    `- [ ] Write tests in prometheus/agents/${name}.test.ts`,
    `- [ ] Run governance scan: prometheus review prometheus/agents/${name}.ts`,
    `- [ ] Add to CI: prometheus agent:audit:log ${name}`,
    '',
    `## Security surface`,
    '',
    `Risk level: ${riskLevel.toUpperCase()}`,
    ...
    `- Log all decisions to Prometheus audit trail`,
    ...
    '',
    `---`,
    `*Generated by prometheus build:agent --plan*`,
    `*Run: prometheus build:agent --scaffold to write code files*`,
  ].join('\n');
}
