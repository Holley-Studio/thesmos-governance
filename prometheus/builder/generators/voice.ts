 ...
 *   - prometheus/agents/voice/<name>/session.ts    — session management
 *   - prometheus/agents/voice/<name>/transport.ts  — audio transport
 *   - prometheus/agents/voice/<name>/pipeline.ts   — STT → LLM → TTS pipeline
 */
...
export function generateVoicePlan(answers: WizardAnswers, context: WizardContext): string {
  ...
  return [
    ...
    `## Files to create`,
    '',
    `- \`prometheus/agents/voice/${name}/session.ts\` — session lifecycle`,
    `- \`prometheus/agents/voice/${name}/transport.ts\` — STT/TTS adapters`,
    `- \`prometheus/agents/voice/${name}/pipeline.ts\` — end-to-end pipeline`,
    '',
    `## Required API keys (all BYOK, never stored by Prometheus)`,
    '',
    ...
    '',
    `---`,
    `*Generated by prometheus build:voice --plan*`,
    `*Run: prometheus build:voice --scaffold to write code files*`,
  ].filter((l) => l !== '').join('\n');
}
...
export async function generateVoice(
  ...
  if (opts.scaffold) {
    files.push({
      path: `prometheus/agents/voice/${name}/session.ts`,
      content: buildSession(answers),
      label: 'Session management',
    });
    files.push({
      path: `prometheus/agents/voice/${name}/transport.ts`,
      content: buildTransport(answers),
      label: 'Audio transport (STT/TTS)',
    });
    files.push({
      path: `prometheus/agents/voice/${name}/pipeline.ts`,
