 ...
 *   - prometheus/mcp-tools/<name>.ts  — tool implementation
 *
 * Registration: add the tool definition + handler to prometheus/mcp-server.ts
 */
 ...
 *
 * To register, add to prometheus/mcp-server.ts:
...
function buildRegistrationSnippet(answers: WizardAnswers): string {
  ...
  return `// ── Add to prometheus/mcp-server.ts ──────────────────────────────────────────
//
// 1. Import at the top:
...
export function generateMcpToolPlan(answers: WizardAnswers, context: WizardContext): string {
  ...
  return [
    ...
    `## Registration steps`,
    '',
    `After creating the file, register in \`prometheus/mcp-server.ts\`:`,
    ...
    '',
    `---`,
    `*Generated by prometheus build:mcp-tool --plan*`,
    `*Run: prometheus build:mcp-tool --scaffold to write code files*`,
  ].filter((l) => l !== '').join('\n');
}
