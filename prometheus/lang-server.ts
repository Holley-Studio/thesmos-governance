/**
 * Prometheus Language Server — LSP 3.17 over stdio.
 *
 * A standalone language server that surfaces Prometheus governance findings
 * as real-time diagnostics in any LSP-compatible editor (VS Code, Cursor,
 * Neovim, Emacs, etc.).
 *
 * Start: prometheus lsp
...
function scanFile(filePath: string, content: string, config: PrometheusConfig): Finding[] {
...
let config: PrometheusConfig = CONFIG_DEFAULTS;
...
function handleInitialize(id: number | string | null): void {
  send({
    ...
    result: {
      ...
      serverInfo: { name: 'prometheus-lsp', version: '1.0.0' },
    },
  });
}
  ...
  const actions = [
    {
      ...
      kind: 'quickfix',
      command: {
        command: 'prometheus.explainRule',
        title: `Explain ${finding.category}`,
        arguments: [finding.category],
      },
    },
    {
      title: `Suppress: prometheus-disable-next-line ${finding.category}`,
      kind: 'quickfix',
      edit: {
        changes: {
          [params.textDocument.uri]: [
            {
              ...
              newText: `// prometheus-disable-next-line ${finding.category}\n`,
            },
          ],
        },
      },
