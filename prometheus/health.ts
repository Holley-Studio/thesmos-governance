/**
 * Prometheus Health Score — a single 0-100 number that synthesises governance state.
 *
...
import type { Finding, ScanResult, PrometheusConfig } from './types.js';
...
export function computeHealthScore(input: HealthInput): HealthScore {
  ...
  if (blockerDrift.length > 0) {
    ...
    priorityActions.push(`Fix BLOCKER drift: ${types}  (run: prometheus drift)`);
  } else if (driftFindings.length > 0) {
    priorityActions.push(`Resolve ${driftFindings.length} drift event${driftFindings.length === 1 ? '' : 's'}  (run: prometheus drift)`);
  }

  // Suppressions
  if (severeSupIssues.length > 0) {
    priorityActions.push(`Remove ${severeSupIssues.length} expired/blanket suppression${severeSupIssues.length === 1 ? '' : 's'}  (run: prometheus suppressions:audit)`);
  }

  // Baseline suggestion
  if (!baseline && findings.length > 0) {
    priorityActions.push(`Create a baseline to track debt separately  (run: prometheus baseline:create)`);
  }

  // Stale report
  if (scan && !reportFresh) {
    priorityActions.push(`Refresh the scan report  (run: prometheus scan)`);
  } else if (!scan) {
    priorityActions.push(`Run a scan first  (run: prometheus scan)`);
  }

  // Remaining HIGH new findings
  const highFindings = newFindings.filter((f) => f.severity === 'HIGH');
  if (highFindings.length > 1) {
    priorityActions.push(`Fix ${highFindings.length - (worstFinding?.severity === 'HIGH' ? 1 : 0)} more HIGH finding${highFindings.length > 2 ? 's' : ''}  (run: prometheus review)`);
  }
...
export function computeHealthForRoot(root: string, config: PrometheusConfig): HealthScore {
...
export function formatHealthConsole(health: HealthScore, projectName = 'Repo'): string {
  ...
  lines.push('');
  lines.push(`  Prometheus Health — ${projectName}`);
  lines.push(WIDE);
  lines.push('');
