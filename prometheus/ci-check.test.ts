...
function makeEmptyInput(configOverride?: Partial<PrometheusConfig>): CiCheckInput {
  return {
...
describe('runCiCheck — all passing', () => {
  ...
  it('includes a check for the GitHub workflow file', () => {
    const checks = runCiCheck(makeFullInput());
    const wf = checks.find((c) => c.name.includes('prometheus-review.yml'));
...
describe('runCiCheck — missing files', () => {
  it('fails when .prometheus/README.md is missing', () => {
    const input = makeFullInput({
      fileExists: (rel) => rel !== '.prometheus/README.md',
    });
    const checks = runCiCheck(input);
    const check = checks.find((c) => c.name === '.prometheus/README.md');
    expect(check?.pass).toBe(false);
    expect(check?.fixHint).toContain('prometheus init');
  });

  it('fails when GitHub workflow is missing', () => {
    const input = makeFullInput({
      fileExists: (rel) => rel !== '.github/workflows/prometheus-review.yml',
    });
    const checks = runCiCheck(input);
    const wf = checks.find((c) => c.name.includes('prometheus-review.yml'));
    expect(wf?.pass).toBe(false);
    expect(wf?.fixHint).toContain('prometheus init');
  });

  it('uses config.github.workflow path when specified', () => {
    const config: PrometheusConfig = {
      ...CONFIG_DEFAULTS,
      github: { workflow: '.github/workflows/custom-prometheus.yml' },
    };
    const allFiles = new Set([
      ...
      '.prometheus/governance/REVIEW_AGENT.md',
      '.prometheus/report.json',
      '.github/workflows/custom-prometheus.yml',
    ]);
    const checks = runCiCheck(makeFullInput({ config, fileExists: (rel) => allFiles.has(rel) }));
    const wf = checks.find((c) => c.name.includes('custom-prometheus.yml'));
    expect(wf?.pass).toBe(true);
  });
...
describe('runCiCheck — stale adapters', () => {
  ...
  it('fails freshness check when version is wrong', () => {
    const oldConfig: PrometheusConfig = { ...CONFIG_DEFAULTS, version: '1.0.0' };
  ...
  it('fails marker check when PROMETHEUS:GENERATED markers are stripped', () => {
    ...
    expect(markerCheck?.fixHint).toContain('prometheus adapters');
  });
...
describe('runCiCheck — config validation', () => {
  ...
  it('fails when config.json is missing required fields', () => {
    const input = makeFullInput({
      readJsonSafe: (rel) =>
        rel === '.prometheus/config.json' ? { project: 'only-project' } : null,
    });
    const check = runCiCheck(input).find((c) => c.name === 'config:valid');
    expect(check?.pass).toBe(false);
...
describe('formatCiCheckConsole', () => {
  ...
  it('includes "CI Check" in header (not "Doctor")', () => {
    ...
    expect(out).not.toContain('Prometheus Doctor');
  });
  ...
  it('shows fix hints for failed checks', () => {
    const checks = runCiCheck(makeEmptyInput());
    const out = formatCiCheckConsole(checks, 'Test');
    expect(out).toContain('prometheus init');
  });
});
