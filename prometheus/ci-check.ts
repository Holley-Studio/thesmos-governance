 ...
 *   5. .prometheus/config.json is valid
 *
 * Does NOT run a full repo scan. Designed to be fast (~ms) and safe to run
 * as an early CI gate before prometheus validate.
 *
    ...
    return {
      ...
      fixHint: pass
        ...
        : relPath.startsWith('.github/')
          ? 'Run prometheus init to generate the GitHub Actions workflow'
          : 'Run prometheus init to create missing Prometheus governance files',
    };
  });
}
  ...
  for (const [target, relPath] of Object.entries(ADAPTER_OUTPUT_PATHS)) {
    ...
    if (content === null) {
      checks.push({
        ...
        fixHint: 'Check file permissions or regenerate with prometheus adapters',
      });
      continue;
    }
    ...
    checks.push({
      ...
      fixHint: hasMarkers
        ? undefined
        : 'Run prometheus adapters to regenerate the file with proper section markers',
    });
  }
  ...
  return [
    {
      ...
      fixHint: valid ? undefined : 'Run prometheus init to create a valid config.json',
    },
  ];
}
...
const CI_TITLE = 'Prometheus CI Check';
...
export function runCiCheckForRoot(root: string, config: PrometheusConfig): DoctorCheck[] {
  return runCiCheck({
    config,
