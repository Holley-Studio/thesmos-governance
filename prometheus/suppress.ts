 ...
 *   // prometheus-disable-next-line <rule-id> -- reason: <text>
 *   // prometheus-disable-next-line <rule-id> -- reason: <text> -- owner: @alice -- expires: 2026-12-31
 *
 ...
 *   // prometheus-disable <rule-id>  (file-scope, no line anchor)
 *
 * Audit categories detected by `prometheus suppressions:audit`:
 ...
 *   blanket         — prometheus-disable-next-line with no rule ID (too broad)
 */
...
const DISABLE_NEXT_LINE_RE = /\/\/\s*prometheus-disable-next-line\s*(.*)/i;
const DISABLE_FILE_RE = /\/\/\s*prometheus-disable\s+(.*)/i;
  ...
  for (const s of suppressions) {
    ...
    if (!s.ruleId) {
      result.push({
        ...
        fixSuggestion: 'Specify a rule ID: prometheus-disable-next-line <rule-id> -- reason: ...',
      });
      continue;
    }
...
export function formatSuppressionAuditConsole(
  ...
  lines.push(`Prometheus Suppressions Audit — ${projectName}`);
  lines.push(SEP);
...
export function formatReviewWithSuppressions(
  ...
  lines.push(`Prometheus Review — ${projectName}`);
  lines.push('');
