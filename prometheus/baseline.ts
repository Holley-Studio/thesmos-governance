/**
 * Prometheus Baseline System — lets large existing repos adopt governance gradually.
 *
 * Workflow:
 *   1. prometheus baseline:create  → snapshot all current findings as known debt
 *   2. CI runs prometheus validate  → only NEW findings (not in baseline) fail the build
 *   3. prometheus baseline:update  → add newly accepted debt; remove fixed findings
 *   4. prometheus baseline:report  → show debt, new violations, and resolved entries
 *
 * Fingerprinting:
...
export const BASELINE_PATH = '.prometheus/baseline.json';
...
export function formatBaselineConsole(
  ...
  const lines: string[] = [];
  const SEP = '─'.repeat(56);
  lines.push(`Prometheus Baseline — ${projectName}`);
  lines.push(SEP);
