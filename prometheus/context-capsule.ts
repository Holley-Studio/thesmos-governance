 ...
 *
 * Generates a compact, authoritative `.prometheus/context.md` that seeds the
...
const CONFIG_FILE      = '.prometheus/config.json';
const REPORT_FILE      = '.prometheus/report.json';
...
function computeHealth(root: string, capsule: Omit<ContextCapsule, 'health'>): ContextHealth {
  ...
  if (existsSync(metaPath)) {
    try {
      ...
      if (contextAgeHours > 48) {
        score -= 20;
        issues.push(`context.md is ${contextAgeHours}h old — run \`prometheus context:snapshot\` to refresh`);
    ...
    score -= 15;
    issues.push('No context snapshot found — run `prometheus context:snapshot` to create one');
  }
  ...
  if (!adaptersFresh) {
    score -= 20;
    issues.push('CLAUDE.md not found — run `prometheus adapters` to generate it');
  }

  // Check governance config
  if (!existsSync(join(root, CONFIG_FILE))) {
    score -= 15;
    issues.push('.prometheus/config.json not found — run `prometheus init` to set up governance');
  }

  // Check for scope config
  if (!existsSync(join(root, '.prometheus', 'scope.json'))) {
    score -= 5;
    issues.push('No scope.json — run `prometheus scope:init` to define agent boundaries');
  }
...
export function renderContextMd(capsule: ContextCapsule): string {
  ...
  lines.push('## Prometheus Governance');
