/**
 * Prometheus registry — loads, validates, and resolves .prometheus/registry.json.
 *
...
export const REGISTRY_PATH = '.prometheus/registry.json';

// ── Types ─────────────────────────────────────────────────────────────────────

/** Shape of .prometheus/registry.json — all fields optional (defaults apply). */
export interface PrometheusRegistryConfig {
  /** Rule pack IDs to enable, e.g. "@prometheus/core", "@prometheus/web". */
  rules?: string[];
  /** Local agent IDs — each maps to .prometheus/agents/<id>.md */
  agents?: string[];
  /** Local skill IDs — each maps to .prometheus/skills/<id>.md */
  skills?: string[];
  /** Profile IDs — used to compose rule packs, agents, and adapters. */
  profiles?: string[];
}

/** A loaded local agent file (from .prometheus/agents/<id>.md). */
export interface AgentEntry {
  id: string;
  /** Parsed from the first `# Heading` in the file. */
  name: string;
  content: string;
  /** Repo-relative path, e.g. `.prometheus/agents/security-reviewer.md` */
  path: string;
}

/** A loaded local skill file (from .prometheus/skills/<id>.md). */
export interface SkillEntry {
  id: string;
  /** Parsed from the first `# Heading` in the file. */
  name: string;
  content: string;
  /** Repo-relative path, e.g. `.prometheus/skills/web-review.md` */
  path: string;
}

/** Fully resolved registry after merging config and loading local files. */
export interface ResolvedRegistry {
  /** Active rule pack IDs. Phase 2 maps these to actual PrometheusRule[]. */
  rulePacks: string[];
...
/** Default registry when no .prometheus/registry.json is present. */
export const REGISTRY_DEFAULTS: Required<PrometheusRegistryConfig> = {
...
export function mergeRegistryConfig(
  base: Required<PrometheusRegistryConfig>,
  override: PrometheusRegistryConfig
): Required<PrometheusRegistryConfig> {
  return {
...
/**
 * Load .prometheus/registry.json from disk.
...
export function loadRegistryConfig(
  ...
  return valid ? (parsed as PrometheusRegistryConfig) : {};
}
...
export function resolveRegistry(
  root: string,
  config: Required<PrometheusRegistryConfig>,
  readFileSafe: (absPath: string) => string | null = defaultReadFileSafe
): ResolvedRegistry {
