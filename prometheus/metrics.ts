/**
 * Prometheus Metrics Engine — local-first, privacy-safe governance analytics.
 *
 * All metrics are derived from existing Prometheus artifacts:
 *   - .prometheus/report.json   (scan results)
 *   - .prometheus/baseline.json (known debt)
 ...
 *   .prometheus/metrics-history.jsonl — one JSON line per CI run.
...
/** Appended to .prometheus/metrics-history.jsonl for trend tracking. */
 ...
 */
export function computeMetrics(input: MetricsInput): PrometheusMetrics {
...
export function collectMetricsForRoot(root: string, config: PrometheusConfig): PrometheusMetrics {
...
export const METRICS_HISTORY_PATH = '.prometheus/metrics-history.jsonl';

/** Convert a full metrics object to the compact snapshot written to history. */
export function toMetricsSnapshot(metrics: PrometheusMetrics): MetricsSnapshot {
  return {
...
export function formatMetricsConsole(metrics: PrometheusMetrics, projectName = 'Repo'): string {
  const lines: string[] = [];
  const SEP = '─'.repeat(56);

  lines.push(`Prometheus Metrics — ${projectName}`);
  lines.push(SEP);
  lines.push('');
...
export function formatMetricsMarkdown(metrics: PrometheusMetrics, projectName = 'Repo'): string {
  const lines: string[] = [];
...
export function formatMetricsJson(metrics: PrometheusMetrics): string {
