...
**(b) Agency lead managing multiple client codebases.** Needs the same governance bar applied consistently across N client repos without re-explaining standards every engagement, plus the creative/production layer (brand, content, campaigns) client work demands. **Pain:** inconsistent review quality across clients erodes trust; no single agent roster covers both code governance and creative delivery. **Why Thesmos:** the CLI's config (`disabledRules`, `reviewIgnorePaths`, baseline) travels with the repo, so the same governance bar ships per-client without re-teaching it; the Agencies Pack ($49, 13 agents — content, brand, campaigns, creative production) is the vertical answer for the non-code half of the job. **Tier fit:** Agencies Pack ($49) + CLI on every client repo. BANT: Budget Medium (agency has real budget, decision is fast at $49), Authority High (lead is the buyer), Need Medium-High, Timeline Medium (next client onboarding cycle). (Roster verified 2026-07-03 against the built `dist-packs/thesmos-pantheon-agencies.zip`: 13 agents, matching pricing.html.)
...
The ladder: **$0 (Starter, 5 agents) → $49 (Founders or Agencies vertical pack) → $79 (Full Pantheon, 67 agents)**, all one-time, no subscription (`website/pricing.html`).
