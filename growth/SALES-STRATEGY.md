...
| 1,137 governance rules, 10 languages | Broad coverage without needing Semgrep's config tuning overhead | `website/vs.html`, `website/pricing.html` — consistent 1,137 figure across both [VERIFY: `docs/gating.md` states "1,138 detections" — a one-rule discrepancy between docs; not material to the sales claim but worth reconciling before print] |
...
| One-time pricing, no subscription | Budget certainty — pay once, no renewal risk, no seat-based creep | `website/pricing.html` — Starter $0 / Full Pantheon $79 / Founders $49 / Agencies $49, all one-time |
...
The ladder: **$0 (Starter, 5 agents) → $49 (Founders or Agencies vertical pack) → $79 (Full Pantheon, 66 agents)**, all one-time, no subscription (`website/pricing.html`).

- **Free tier exists to remove all friction from first activation.** No email, no card (`pricing.html` FAQ: "No account, no email, no credit card"). This is the top of every channel in §4 — the free CLI and free Starter Pack are the thing every piece of content points at.
...
Channel (growth/reddit, growth/x-threads, growth/product-hunt, growth/shorts, growth/medium)
  ...
  → Free tier: CLI (npm install) OR Starter Pack zip (5 agents, no email required)
  → Email capture: Starter Pack purchase on Gumroad triggers the 3-email workflow
  ...
  → Paid: Full Pantheon ($79) or a $49 vertical pack via Gumroad
```
...
1. **"Isn't this just prompts/agent files anyone could write?"** — Acknowledge: yes, the persona markdown is readable and open (source content for every god is public on GitHub). Explore: what you're actually paying for isn't the prompt text, it's the packaging — pre-built Zeus orchestrators per platform, platform-native exports for all 10 supported tools, one-click installs, and lifetime updates (`pricing.html` "open-core" explainer). Reframe: the free Starter Pack proves this — if the personas alone were the value, we wouldn't give 5 of them away for $0. Close: "Try the free tier first. If the packaging and orchestration don't save you time versus hand-rolling it, don't buy the rest."
